...
public class FacturaController implements Serializable {
    ...
    public Factura getSelected() {
        if (current == null) {
            current = new Factura();
            current.setFacturaPK(new conexion.FacturaPK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public String create() {
        try {
            current.getFacturaPK().setLINEAidLINEA(current.getLinea().getLineaPK().getIdLINEA());
    ...
    public Factura getFactura(conexion.FacturaPK id) {
    ...
    public static class FacturaControllerConverter implements Converter {
        ...
        conexion.FacturaPK getKey(String value) {
            conexion.FacturaPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new conexion.FacturaPK();
            key.setIdFACTURA(Integer.parseInt(values[0]));
            key.setLINEAidLINEA(Integer.parseInt(values[1]));
            return key;
        }

        String getStringKey(conexion.FacturaPK value) {
